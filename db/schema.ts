...
export const investigationContextEvent = pgTable(
  'investigation_context_event',
  {
    id: text('id').primaryKey(),
    investigationId: text('investigation_id').notNull().references(() => investigation.id, { onDelete: 'cascade' }),
    aiInvestigationId: text('ai_investigation_id').references((): AnyPgColumn => aiInvestigation.id, { onDelete: 'restrict' }),
    aiCandidateGenerationId: text('ai_candidate_generation_id').references((): AnyPgColumn => aiCandidateGeneration.id, { onDelete: 'restrict' }),
    workspaceId: text('workspace_id').notNull().references(() => workspace.id, { onDelete: 'cascade' }),
    operation: text('operation').notNull(),
    status: text('status').notNull(),
  ...
  (table) => [
    ...
    index('investigation_context_event_ai_idx').on(table.aiInvestigationId, table.createdAt),
    index('investigation_context_event_ai_candidate_generation_idx').on(table.aiCandidateGenerationId, table.createdAt),
...
export const candidateVerification = pgTable(
  'candidate_verification',
  {
    ...
    formatVersion: integer('format_version').notNull(),
    state: text('state').notNull(),
    candidateArtifactIntegrity: text('candidate_artifact_integrity'),
    verificationContract: text('verification_contract'),
    baselineComparison: text('baseline_comparison'),
    repairObjectiveEvidence: text('repair_objective_evidence').notNull().default('not_measured'),
    evidenceId: text('evidence_id').unique().references((): AnyPgColumn => candidateVerificationEvidence.id, { onDelete: 'restrict' }),
    failureCode: text('failure_code'),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  ...
  (table) => [
    index('candidate_verification_workspace_created_idx').on(table.workspaceId, table.createdAt),
    index('candidate_verification_state_idx').on(table.state, table.updatedAt),
    uniqueIndex('candidate_verification_active_candidate_unique').on(table.candidateId).where(sql`${table.state} in ('created','queued','verifying')`),
    check('candidate_verification_id_check', sql`${table.id} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'`),
    check('candidate_verification_commit_check', sql`${table.baseCommitSha} ~ '^[0-9a-f]{40}$'`),
    check('candidate_verification_hashes_check', sql`${table.profileIdentity} ~ '^[0-9a-f]{64}$' and ${table.candidateIdentity} ~ '^[0-9a-f]{64}$'`),
    check('candidate_verification_version_check', sql`${table.formatVersion} = 1`),
    check('candidate_verification_state_check', sql`${table.state} in ('created','queued','verifying','completed','infrastructure_failed','cancelled')`),
    check('candidate_verification_results_check', sql`(${table.candidateArtifactIntegrity} is null or ${table.candidateArtifactIntegrity} in ('valid','invalid')) and (${table.verificationContract} is null or ${table.verificationContract} in ('checks_passed','checks_failed','infrastructure_failed')) and (${table.baselineComparison} is null or ${table.baselineComparison} in ('no_regression_detected','regression_detected','previous_baseline_failure_resolved','previous_baseline_failure_still_present','not_comparable')) and ${table.repairObjectiveEvidence} = 'not_measured' and (${table.failureCode} is null or ${table.failureCode} ~ '^[a-z_]{1,64}$')`),
    check('candidate_verification_state_facts_check', sql`(${table.state} in ('created','queued') and ${table.verificationStartedAt} is null and ${table.completedAt} is null and ${table.evidenceId} is null and ${table.candidateArtifactIntegrity} is null and ${table.verificationContract} is null and ${table.baselineComparison} is null and ${table.failureCode} is null) or (${table.state} = 'verifying' and ${table.verificationStartedAt} is not null and ${table.completedAt} is null and ${table.evidenceId} is null and ${table.verificationContract} is null and ${table.baselineComparison} is null) or (${table.state} = 'completed' and ${table.verificationStartedAt} is not null and ${table.completedAt} is not null and ${table.evidenceId} is not null and ${table.candidateArtifactIntegrity} = 'valid' and ${table.verificationContract} in ('checks_passed','checks_failed') and ${table.baselineComparison} is not null and ${table.failureCode} is null) or (${table.state} = 'infrastructure_failed' and ${table.verificationStartedAt} is not null and ${table.completedAt} is not null and ${table.verificationContract} = 'infrastructure_failed' and ${table.baselineComparison} = 'not_comparable' and ${table.failureCode} is not null) or (${table.state} = 'cancelled' and ${table.completedAt} is not null and ${table.baselineComparison} = 'not_comparable' and ${table.failureCode} is not null))`),
  ],
);

export const candidateVerificationAttempt = pgTable(
  'candidate_verification_attempt',
  {
    id: text('id').primaryKey(),
    verificationId: text('verification_id').notNull().references(() => candidateVerification.id, { onDelete: 'restrict' }),
    queueJobId: text('queue_job_id').notNull(),
    attemptNumber: integer('attempt_number').notNull(),
    expectedEvidenceId: text('expected_evidence_id').notNull().unique(),
    ownershipToken: text('ownership_token').notNull(),
    state: text('state').notNull(),
    claimedAt: timestamp('claimed_at', { withTimezone: true }).notNull(),
    heartbeatAt: timestamp('heartbeat_at', { withTimezone: true }).notNull(),
  ...
  (table) => [
    uniqueIndex('candidate_verification_attempt_number_unique').on(table.verificationId, table.attemptNumber),
    uniqueIndex('candidate_verification_attempt_active_unique').on(table.verificationId).where(sql`${table.state} = 'active'`),
    index('candidate_verification_attempt_verification_idx').on(table.verificationId, table.claimedAt),
    check('candidate_verification_attempt_uuid_check', sql`${table.id} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' and ${table.queueJobId} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' and ${table.expectedEvidenceId} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' and ${table.ownershipToken} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'`),
    check('candidate_verification_attempt_number_check', sql`${table.attemptNumber} between 1 and 3`),
    check('candidate_verification_attempt_state_check', sql`${table.state} in ('active','succeeded','checks_failed','retryable_failed','exhausted','abandoned')`),
    check('candidate_verification_attempt_lease_check', sql`(${table.state} = 'active' and ${table.leaseExpiresAt} is not null and ${table.finishedAt} is null) or (${table.state} <> 'active' and ${table.leaseExpiresAt} is null and ${table.finishedAt} is not null)`),
    check('candidate_verification_attempt_sandbox_check', sql`(${table.sandboxName} is null or ${table.sandboxName} ~ '^[a-z0-9][a-z0-9-]{0,99}$') and (${table.sandboxSessionId} is null or ${table.sandboxSessionId} ~ '^[A-Za-z0-9_-]{1,128}$')`),
    check('candidate_verification_attempt_cleanup_check', sql`(${table.cleanupStop} is null or ${table.cleanupStop} in ('confirmed','failed','not_needed')) and (${table.cleanupDelete} is null or ${table.cleanupDelete} in ('confirmed','failed','not_needed')) and (${table.cleanupLookup} is null or ${table.cleanupLookup} in ('absent','still_present','unconfirmed','unconfirmed_after_create_failure','not_run')) and (${table.failureCode} is null or ${table.failureCode} ~ '^[a-z_]{1,64}$')`),
  ],
);

export const candidateVerificationEvidence = pgTable(
  'candidate_verification_evidence',
  {
    id: text('id').primaryKey(),
    verificationId: text('verification_id').notNull().references(() => candidateVerification.id, { onDelete: 'restrict' }),
    attemptId: text('attempt_id').notNull().unique().references(() => candidateVerificationAttempt.id, { onDelete: 'restrict' }),
    evidenceVersion: integer('evidence_version').notNull(),
    candidateId: text('candidate_id').notNull().references(() => repairCandidate.id, { onDelete: 'restrict' }),
    candidateIdentity: text('candidate_identity').notNull(),
    workspaceId: text('workspace_id').notNull().references(() => workspace.id, { onDelete: 'restrict' }),
    ...
    profileIdentity: text('profile_identity').notNull(),
    baselineId: text('baseline_id').notNull().references(() => repositoryBaseline.id, { onDelete: 'restrict' }),
    candidateArtifactIntegrity: text('candidate_artifact_integrity').notNull(),
    sandboxName: text('sandbox_name').notNull(),
    sandboxSessionId: text('sandbox_session_id'),
    distinctSandboxConfirmed: boolean('distinct_sandbox_confirmed').notNull(),
    pristineSourceIdentity: text('pristine_source_identity'),
    pristineBaseIntegrity: text('pristine_base_integrity').notNull(),
    reconstructedSourceIdentity: text('reconstructed_source_identity'),
    candidateReconstruction: text('candidate_reconstruction').notNull(),
    credentialsExposure: text('credentials_exposure').notNull(),
    networkPolicy: text('network_policy').notNull(),
    installStatus: text('install_status').notNull(),
    installExitCode: integer('install_exit_code'),
    ...
    testTimedOut: boolean('test_timed_out').notNull(),
    sourceIdentityAfter: text('source_identity_after'),
    sourceIntegrityUnchanged: boolean('source_integrity_unchanged'),
    cleanupStop: text('cleanup_stop').notNull(),
    cleanupDelete: text('cleanup_delete').notNull(),
    cleanupLookup: text('cleanup_lookup').notNull(),
    executionOutcome: text('execution_outcome').notNull(),
    verificationContract: text('verification_contract').notNull(),
    baselineComparison: text('baseline_comparison').notNull(),
    repairObjectiveEvidence: text('repair_objective_evidence').notNull(),
    errorPhase: text('error_phase'),
    errorCode: text('error_code'),
    startedAt: timestamp('started_at', { withTimezone: true }).notNull(),
    completedAt: timestamp('completed_at', { withTimezone: true }).notNull(),
  ...
  (table) => [
    index('candidate_verification_evidence_verification_idx').on(table.verificationId, table.completedAt),
    check('candidate_verification_evidence_version_check', sql`${table.evidenceVersion} = 1`),
    check('candidate_verification_evidence_hash_check', sql`${table.candidateIdentity} ~ '^[0-9a-f]{64}$' and ${table.profileIdentity} ~ '^[0-9a-f]{64}$' and ${table.baseCommitSha} ~ '^[0-9a-f]{40}$' and (${table.pristineSourceIdentity} is null or ${table.pristineSourceIdentity} ~ '^[0-9a-f]{64}$') and (${table.reconstructedSourceIdentity} is null or ${table.reconstructedSourceIdentity} ~ '^[0-9a-f]{64}$') and (${table.sourceIdentityAfter} is null or ${table.sourceIdentityAfter} ~ '^[0-9a-f]{64}$')`),
    check('candidate_verification_evidence_integrity_check', sql`${table.candidateArtifactIntegrity} in ('valid','invalid') and ${table.pristineBaseIntegrity} in ('valid','invalid','not_checked') and ${table.candidateReconstruction} in ('valid','invalid','not_checked') and ${table.credentialsExposure} in ('absent','present','not_checked') and ${table.networkPolicy} in ('deny-all','unconfirmed')`),
    check('candidate_verification_evidence_phase_check', sql`${table.installStatus} in ('not_run','completed','failed','timed_out') and (${table.typecheckStatus} is null or ${table.typecheckStatus} in ('not_run','completed','failed','timed_out')) and (${table.buildStatus} is null or ${table.buildStatus} in ('not_run','completed','failed','timed_out')) and ${table.testStatus} in ('not_run','completed','failed','timed_out')`),
    check('candidate_verification_evidence_cleanup_check', sql`${table.cleanupStop} in ('confirmed','failed','not_needed') and ${table.cleanupDelete} in ('confirmed','failed','not_needed') and ${table.cleanupLookup} in ('absent','still_present','unconfirmed','unconfirmed_after_create_failure','not_run')`),
    check('candidate_verification_evidence_result_check', sql`${table.executionOutcome} in ('checks_passed','typecheck_failed','build_failed','test_failed','installation_failed','timed_out','cancelled','infrastructure_failed','cleanup_failed','artifact_invalid') and ${table.verificationContract} in ('checks_passed','checks_failed','infrastructure_failed') and ${table.baselineComparison} in ('no_regression_detected','regression_detected','previous_baseline_failure_resolved','previous_baseline_failure_still_present','not_comparable') and ${table.repairObjectiveEvidence} = 'not_measured' and (${table.errorCode} is null or ${table.errorCode} ~ '^[a-z_]{1,64}$') and ${table.durationMs} >= 0`),
    check('candidate_verification_evidence_contract_trust_check', sql`${table.verificationContract} = 'infrastructure_failed' or (${table.candidateArtifactIntegrity} = 'valid' and ${table.distinctSandboxConfirmed} is true and ${table.pristineBaseIntegrity} = 'valid' and ${table.candidateReconstruction} = 'valid' and ${table.credentialsExposure} = 'absent' and ${table.networkPolicy} = 'deny-all' and ${table.installStatus} = 'completed' and ${table.installExitCode} = 0 and ${table.installTimedOut} is false and ${table.reconstructedSourceIdentity} = ${table.sourceIdentityAfter} and ${table.sourceIntegrityUnchanged} is true and ${table.cleanupStop} = 'confirmed' and ${table.cleanupDelete} = 'confirmed' and ${table.cleanupLookup} = 'absent')`),
    check('candidate_verification_evidence_clean_success_check', sql`${table.verificationContract} <> 'checks_passed' or (${table.candidateArtifactIntegrity} = 'valid' and ${table.distinctSandboxConfirmed} is true and ${table.pristineBaseIntegrity} = 'valid' and ${table.candidateReconstruction} = 'valid' and ${table.credentialsExposure} = 'absent' and ${table.networkPolicy} = 'deny-all' and ${table.installStatus} = 'completed' and ${table.installExitCode} = 0 and ${table.installTimedOut} is false and (${table.typecheckStatus} is null or (${table.typecheckStatus} = 'completed' and ${table.typecheckExitCode} = 0 and ${table.typecheckTimedOut} is false)) and (${table.buildStatus} is null or (${table.buildStatus} = 'completed' and ${table.buildExitCode} = 0 and ${table.buildTimedOut} is false)) and ${table.testStatus} = 'completed' and ${table.testExitCode} = 0 and ${table.testTimedOut} is false and ${table.reconstructedSourceIdentity} = ${table.sourceIdentityAfter} and ${table.sourceIntegrityUnchanged} is true and ${table.cleanupStop} = 'confirmed' and ${table.cleanupDelete} = 'confirmed' and ${table.cleanupLookup} = 'absent' and ${table.errorCode} is null)`),
  ],
);

export const candidateVerificationEvent = pgTable(
  ...
  (table) => [
    index('candidate_verification_event_verification_idx').on(table.verificationId, table.createdAt),
    check('candidate_verification_event_state_check', sql`(${table.fromState} is null or ${table.fromState} in ('created','queued','verifying','completed','infrastructure_failed','cancelled')) and ${table.toState} in ('created','queued','verifying','completed','infrastructure_failed','cancelled')`),
    check('candidate_verification_event_transition_check', sql`(${table.fromState} is null and ${table.toState} = 'created') or (${table.fromState} = 'created' and ${table.toState} in ('queued','cancelled')) or (${table.fromState} = 'queued' and ${table.toState} in ('verifying','cancelled')) or (${table.fromState} = 'verifying' and ${table.toState} in ('completed','infrastructure_failed','cancelled'))`),
    check('candidate_verification_event_safe_check', sql`(${table.verificationContract} is null or ${table.verificationContract} in ('checks_passed','checks_failed','infrastructure_failed')) and (${table.baselineComparison} is null or ${table.baselineComparison} in ('no_regression_detected','regression_detected','previous_baseline_failure_resolved','previous_baseline_failure_still_present','not_comparable')) and (${table.failureCode} is null or ${table.failureCode} ~ '^[a-z_]{1,64}$')`),
  ],
);

export const aiInvestigation = pgTable(
  'ai_investigation',
  {
    id: text('id').primaryKey(),
    investigationId: text('investigation_id').notNull().references(() => investigation.id, { onDelete: 'restrict' }),
    ...
    conclusionStatus: text('conclusion_status'),
    summary: text('summary'),
    suspectedFiles: jsonb('suspected_files').$type<Array<{ path: string; reason: string }>>(),
    evidenceReferences: jsonb('evidence_references').$type<Array<{ kind: 'baseline' | 'file' | 'search'; reference: string }>>(),
  ...
  (table) => [
    uniqueIndex('ai_investigation_ordinal_unique').on(table.investigationId, table.executionOrdinal),
    uniqueIndex('ai_investigation_idempotency_unique').on(table.investigationId, table.idempotencyKey),
    uniqueIndex('ai_investigation_active_unique').on(table.investigationId).where(sql`${table.state} in ('created','queued','investigating')`),
    index('ai_investigation_workspace_created_idx').on(table.workspaceId, table.createdAt),
    index('ai_investigation_state_idx').on(table.state, table.updatedAt),
    check('ai_investigation_uuid_check', sql`${table.id} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'`),
    check('ai_investigation_execution_check', sql`${table.executionOrdinal} >= 1 and ${table.idempotencyKey} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'`),
    check('ai_investigation_commit_check', sql`${table.baseCommitSha} ~ '^[0-9a-f]{40}$' and ${table.profileIdentity} ~ '^[0-9a-f]{64}$'`),
    check('ai_investigation_provider_check', sql`char_length(${table.providerId}) between 1 and 40 and char_length(${table.modelId}) between 1 and 80 and ${table.protocolVersion} = 1`),
    check('ai_investigation_state_check', sql`${table.state} in ('created','queued','investigating','completed','failed','cancelled')`),
    check('ai_investigation_completion_check', sql`(${table.completionReason} is null or ${table.completionReason} in ('model_conclusion','budget_exhausted')) and (${table.conclusionStatus} is null or ${table.conclusionStatus} in ('diagnosis_found','insufficient_evidence','objective_not_reproduced')) and (${table.confidence} is null or ${table.confidence} in ('low','medium','high'))`),
    check('ai_investigation_usage_check', sql`${table.inputTokens} between 0 and 1000000 and ${table.outputTokens} between 0 and 100000 and ${table.toolCallCount} between 0 and 20 and ${table.modelTurnCount} between 0 and 8`),
    check('ai_investigation_output_check', sql`(${table.summary} is null or char_length(${table.summary}) between 1 and 1200) and (${table.proposedApproach} is null or char_length(${table.proposedApproach}) between 1 and 1600) and (${table.suspectedFiles} is null or (jsonb_typeof(${table.suspectedFiles}) = 'array' and jsonb_array_length(${table.suspectedFiles}) <= 8)) and (${table.evidenceReferences} is null or (jsonb_typeof(${table.evidenceReferences}) = 'array' and jsonb_array_length(${table.evidenceReferences}) <= 12)) and (${table.failureCode} is null or ${table.failureCode} ~ '^[a-z_]{1,64}$')`),
    check('ai_investigation_evidence_check', sql`${table.completionReason} <> 'model_conclusion' or (jsonb_typeof(${table.evidenceReferences}) = 'array' and jsonb_array_length(${table.evidenceReferences}) between 1 and 12 and (${table.conclusionStatus} <> 'diagnosis_found' or jsonb_array_length(${table.suspectedFiles}) between 1 and 8))`),
    check('ai_investigation_state_facts_check', sql`(${table.state} = 'created' and ${table.queuedAt} is null and ${table.investigationStartedAt} is null and ${table.completedAt} is null and ${table.conclusionStatus} is null and ${table.failureCode} is null) or (${table.state} = 'queued' and ${table.queuedAt} is not null and ${table.investigationStartedAt} is null and ${table.completedAt} is null and ${table.conclusionStatus} is null and ${table.failureCode} is null) or (${table.state} = 'investigating' and ${table.queuedAt} is not null and ${table.investigationStartedAt} is not null and ${table.completedAt} is null and ${table.conclusionStatus} is null) or (${table.state} = 'completed' and ${table.queuedAt} is not null and ${table.investigationStartedAt} is not null and ${table.completedAt} is not null and ${table.completionReason} is not null and ${table.conclusionStatus} is not null and ${table.summary} is not null and ${table.suspectedFiles} is not null and ${table.evidenceReferences} is not null and ${table.proposedApproach} is not null and ${table.confidence} is not null and ${table.failureCode} is null) or (${table.state} in ('failed','cancelled') and ${table.queuedAt} is not null and ${table.completedAt} is not null and ${table.failureCode} is not null)`),
  ],
);

export const aiInvestigationAttempt = pgTable(
  'ai_investigation_attempt',
  {
    id: text('id').primaryKey(),
  ...
  (table) => [
    uniqueIndex('ai_investigation_attempt_number_unique').on(table.aiInvestigationId, table.attemptNumber),
    uniqueIndex('ai_investigation_attempt_active_unique').on(table.aiInvestigationId).where(sql`${table.state} = 'active'`),
    check('ai_investigation_attempt_uuid_check', sql`${table.id} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' and ${table.queueJobId} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' and ${table.ownershipToken} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'`),
    check('ai_investigation_attempt_number_check', sql`${table.attemptNumber} between 1 and 3`),
    check('ai_investigation_attempt_state_check', sql`${table.state} in ('active','succeeded','retryable_failed','exhausted','abandoned')`),
    check('ai_investigation_attempt_lease_check', sql`(${table.state} = 'active' and ${table.leaseExpiresAt} is not null and ${table.finishedAt} is null) or (${table.state} <> 'active' and ${table.leaseExpiresAt} is null and ${table.finishedAt} is not null)`),
    check('ai_investigation_attempt_failure_check', sql`(${table.failureCode} is null or ${table.failureCode} ~ '^[a-z_]{1,64}$') and ((${table.state} = 'succeeded' and ${table.failureCode} is null) or (${table.state} <> 'succeeded'))`),
  ],
);

export const aiInvestigationEvent = pgTable(
  'ai_investigation_event',
  {
    id: text('id').primaryKey(),
    aiInvestigationId: text('ai_investigation_id').notNull().references(() => aiInvestigation.id, { onDelete: 'restrict' }),
    workspaceId: text('workspace_id').notNull().references(() => workspace.id, { onDelete: 'restrict' }),
  ...
  (table) => [
    index('ai_investigation_event_idx').on(table.aiInvestigationId, table.createdAt),
    check('ai_investigation_event_uuid_check', sql`${table.id} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'`),
    check('ai_investigation_event_state_check', sql`(${table.fromState} is null or ${table.fromState} in ('created','queued','investigating','completed','failed','cancelled')) and ${table.toState} in ('created','queued','investigating','completed','failed','cancelled')`),
    check('ai_investigation_event_transition_check', sql`(${table.fromState} is null and ${table.toState} = 'created') or (${table.fromState} = 'created' and ${table.toState} in ('queued','cancelled')) or (${table.fromState} = 'queued' and ${table.toState} in ('investigating','cancelled')) or (${table.fromState} = 'investigating' and ${table.toState} in ('completed','failed','cancelled'))`),
    check('ai_investigation_event_failure_check', sql`(${table.failureCode} is null or ${table.failureCode} ~ '^[a-z_]{1,64}$') and ((${table.toState} in ('failed','cancelled') and ${table.failureCode} is not null) or (${table.toState} not in ('failed','cancelled') and ${table.failureCode} is null))`),
  ],
);

export const aiCandidateGeneration = pgTable(
  'ai_candidate_generation',
  {
    id: text('id').primaryKey(),
    aiInvestigationId: text('ai_investigation_id').notNull().references(() => aiInvestigation.id, { onDelete: 'restrict' }),
    investigationId: text('investigation_id').notNull().references(() => investigation.id, { onDelete: 'restrict' }),
    ...
    idempotencyKey: text('idempotency_key').notNull(),
    state: text('state').notNull(),
    repairCandidateId: text('repair_candidate_id').references(() => repairCandidate.id, { onDelete: 'restrict' }),
    completionReason: text('completion_reason'),
    inputTokens: integer('input_tokens').notNull().default(0),
    outputTokens: integer('output_tokens').notNull().default(0),
    toolCallCount: integer('tool_call_count').notNull().default(0),
  ...
  (table) => [
    uniqueIndex('ai_candidate_generation_execution_ordinal_unique').on(table.aiInvestigationId, table.executionOrdinal),
    uniqueIndex('ai_candidate_generation_active_unique').on(table.aiInvestigationId).where(sql`${table.state} in ('created','queued','generating')`),
    uniqueIndex('ai_candidate_generation_idempotency_unique').on(table.aiInvestigationId, table.idempotencyKey),
    index('ai_candidate_generation_workspace_created_idx').on(table.workspaceId, table.createdAt),
    index('ai_candidate_generation_state_idx').on(table.state, table.updatedAt),
    check('ai_candidate_generation_uuid_check', sql`${table.id} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' and ${table.idempotencyKey} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'`),
    check('ai_candidate_generation_commit_check', sql`${table.baseCommitSha} ~ '^[0-9a-f]{40}$' and ${table.profileIdentity} ~ '^[0-9a-f]{64}$'`),
    check('ai_candidate_generation_provider_check', sql`char_length(${table.providerId}) between 1 and 40 and char_length(${table.modelId}) between 1 and 80 and ${table.protocolVersion} in (1,2,3,4)`),
    check('ai_candidate_generation_execution_ordinal_check', sql`${table.executionOrdinal} >= 1`),
    check('ai_candidate_generation_state_check', sql`${table.state} in ('created','queued','generating','frozen','abstained','failed','cancelled')`),
    check('ai_candidate_generation_usage_check', sql`${table.inputTokens} between 0 and 1000000 and ${table.outputTokens} between 0 and 100000 and ${table.toolCallCount} between 0 and 5 and ${table.modelTurnCount} between 0 and 7`),
    check('ai_candidate_generation_result_check', sql`(${table.failureCode} is null or ${table.failureCode} ~ '^[a-z_]{1,64}$') and ((${table.state} = 'frozen' and ${table.repairCandidateId} is not null and ${table.failureCode} is null and ${table.completionReason} = 'proposal_ready') or (${table.state} = 'abstained' and ${table.repairCandidateId} is null and ${table.failureCode} is null and ${table.completionReason} = 'insufficient_evidence') or (${table.state} not in ('frozen','abstained') and ${table.repairCandidateId} is null and ${table.completionReason} is null))`),
    check('ai_candidate_generation_state_facts_check', sql`(${table.state} = 'created' and ${table.queuedAt} is null and ${table.generationStartedAt} is null and ${table.completedAt} is null) or (${table.state} = 'queued' and ${table.queuedAt} is not null and ${table.generationStartedAt} is null and ${table.completedAt} is null) or (${table.state} = 'generating' and ${table.queuedAt} is not null and ${table.generationStartedAt} is not null and ${table.completedAt} is null) or (${table.state} in ('frozen','abstained','failed','cancelled') and ${table.queuedAt} is not null and ${table.generationStartedAt} is not null and ${table.completedAt} is not null)`),
  ],
);

export const aiCandidateGenerationAttempt = pgTable(
  'ai_candidate_generation_attempt',
  {
    id: text('id').primaryKey(),
    generationId: text('generation_id').notNull().references(() => aiCandidateGeneration.id, { onDelete: 'restrict' }),
    queueJobId: text('queue_job_id').notNull(),
    attemptNumber: integer('attempt_number').notNull(),
    ownershipToken: text('ownership_token').notNull(),
    state: text('state').notNull(),
  ...
  (table) => [
    uniqueIndex('ai_candidate_generation_attempt_number_unique').on(table.generationId, table.attemptNumber),
    uniqueIndex('ai_candidate_generation_attempt_active_unique').on(table.generationId).where(sql`${table.state} = 'active'`),
    check('ai_candidate_generation_attempt_uuid_check', sql`${table.id} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' and ${table.queueJobId} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' and ${table.ownershipToken} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'`),
    check('ai_candidate_generation_attempt_number_check', sql`${table.attemptNumber} between 1 and 3`),
    check('ai_candidate_generation_attempt_state_check', sql`${table.state} in ('active','succeeded','retryable_failed','exhausted','abandoned')`),
    check('ai_candidate_generation_attempt_lease_check', sql`(${table.state} = 'active' and ${table.leaseExpiresAt} is not null and ${table.finishedAt} is null) or (${table.state} <> 'active' and ${table.leaseExpiresAt} is null and ${table.finishedAt} is not null)`),
    check('ai_candidate_generation_attempt_failure_check', sql`(${table.failureCode} is null or ${table.failureCode} ~ '^[a-z_]{1,64}$') and ((${table.state} = 'succeeded' and ${table.failureCode} is null) or (${table.state} <> 'succeeded'))`),
  ],
);

export const aiCandidateGenerationEvent = pgTable(
  'ai_candidate_generation_event',
  {
    id: text('id').primaryKey(),
    generationId: text('generation_id').notNull().references(() => aiCandidateGeneration.id, { onDelete: 'restrict' }),
    workspaceId: text('workspace_id').notNull().references(() => workspace.id, { onDelete: 'restrict' }),
  ...
  (table) => [
    index('ai_candidate_generation_event_idx').on(table.generationId, table.createdAt),
    check('ai_candidate_generation_event_uuid_check', sql`${table.id} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'`),
    check('ai_candidate_generation_event_state_check', sql`(${table.fromState} is null or ${table.fromState} in ('created','queued','generating','frozen','abstained','failed','cancelled')) and ${table.toState} in ('created','queued','generating','frozen','abstained','failed','cancelled')`),
    check('ai_candidate_generation_event_transition_check', sql`(${table.fromState} is null and ${table.toState} = 'created') or (${table.fromState} = 'created' and ${table.toState} in ('queued','cancelled')) or (${table.fromState} = 'queued' and ${table.toState} in ('generating','cancelled')) or (${table.fromState} = 'generating' and ${table.toState} in ('frozen','abstained','failed','cancelled'))`),
    check('ai_candidate_generation_event_failure_check', sql`(${table.failureCode} is null or ${table.failureCode} ~ '^[a-z_]{1,64}$') and ((${table.toState} in ('failed','cancelled') and ${table.failureCode} is not null) or (${table.toState} not in ('failed','cancelled') and ${table.failureCode} is null))`),
  ],
);
...
export const humanReviewDecision = pgTable(
  'human_review_decision',
  {
    ...
    repairRunId: text('repair_run_id').notNull().references(() => repairRun.id, { onDelete: 'restrict' }),
    repairLoopId: text('repair_loop_id').notNull().unique().references(() => repairLoop.id, { onDelete: 'restrict' }),
    repairLoopIterationId: text('repair_loop_iteration_id').notNull().references(() => repairLoopIteration.id, { onDelete: 'restrict' }),
    aiCandidateGenerationId: text('ai_candidate_generation_id').notNull().references(() => aiCandidateGeneration.id, { onDelete: 'restrict' }),
    repairCandidateId: text('repair_candidate_id').notNull().references(() => repairCandidate.id, { onDelete: 'restrict' }),
    candidateIdentity: text('candidate_identity').notNull(),
    candidateVerificationId: text('candidate_verification_id').notNull().references(() => candidateVerification.id, { onDelete: 'restrict' }),
    verificationEvidenceId: text('verification_evidence_id').notNull().references(() => candidateVerificationEvidence.id, { onDelete: 'restrict' }),
    githubRepositoryId: bigint('github_repository_id', { mode: 'number' }).notNull(),
    installationId: bigint('installation_id', { mode: 'number' }).notNull(),
    baselineId: text('baseline_id').notNull().references(() => repositoryBaseline.id, { onDelete: 'restrict' }),
    baseCommitSha: text('base_commit_sha').notNull(),
    profileIdentity: text('profile_identity').notNull(),
    objectiveContractHash: text('objective_contract_hash').notNull(),
    objectiveEvidenceHash: text('objective_evidence_hash').notNull(),
    verificationEvidenceIdentity: text('verification_evidence_identity').notNull(),
    reviewSubjectIdentity: text('review_subject_identity').notNull(),
    decision: text('decision').notNull(),
    decisionIdentity: text('decision_identity').notNull().unique(),
    idempotencyKey: text('idempotency_key').notNull(),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => [
    uniqueIndex('human_review_workspace_idempotency_unique').on(table.workspaceId, table.idempotencyKey),
    index('human_review_workspace_created_idx').on(table.workspaceId, table.createdAt),
    index('human_review_repair_run_idx').on(table.repairRunId, table.createdAt),
    check('human_review_uuid_check', sql`${table.id} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' and ${table.idempotencyKey} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'`),
    check('human_review_version_check', sql`${table.version} = 1`),
    check('human_review_decision_check', sql`${table.decision} in ('approved','rejected')`),
    check('human_review_commit_check', sql`${table.baseCommitSha} ~ '^[0-9a-f]{40}$'`),
    check('human_review_hashes_check', sql`${table.candidateIdentity} ~ '^[0-9a-f]{64}$' and ${table.profileIdentity} ~ '^[0-9a-f]{64}$' and ${table.objectiveContractHash} ~ '^[0-9a-f]{64}$' and ${table.objectiveEvidenceHash} ~ '^[0-9a-f]{64}$' and ${table.verificationEvidenceIdentity} ~ '^[0-9a-f]{64}$' and ${table.reviewSubjectIdentity} ~ '^[0-9a-f]{64}$' and ${table.decisionIdentity} ~ '^[0-9a-f]{64}$'`),
  ],
);

export const repairPublication = pgTable(
  'repair_publication',
  {
    id: text('id').primaryKey(),
    version: integer('version').notNull(),
    ...
    repairRunId: text('repair_run_id').notNull().unique().references(() => repairRun.id, { onDelete: 'restrict' }),
    repairLoopId: text('repair_loop_id').notNull().unique().references(() => repairLoop.id, { onDelete: 'restrict' }),
    humanReviewDecisionId: text('human_review_decision_id').notNull().unique().references(() => humanReviewDecision.id, { onDelete: 'restrict' }),
    humanReviewDecisionIdentity: text('human_review_decision_identity').notNull(),
    repairCandidateId: text('repair_candidate_id').notNull().references(() => repairCandidate.id, { onDelete: 'restrict' }),
    candidateIdentity: text('candidate_identity').notNull(),
    candidateVerificationId: text('candidate_verification_id').notNull().references(() => candidateVerification.id, { onDelete: 'restrict' }),
    verificationEvidenceId: text('verification_evidence_id').notNull().references(() => candidateVerificationEvidence.id, { onDelete: 'restrict' }),
    ...
    objectiveContractHash: text('objective_contract_hash').notNull(),
    objectiveEvidenceHash: text('objective_evidence_hash').notNull(),
    repairIntentId: text('repair_intent_id').notNull().references(() => repairIntent.id, { onDelete: 'restrict' }),
    repairObjective: text('repair_objective').notNull(),
    repairObjectiveHash: text('repair_objective_hash').notNull(),
    ...
    pullRequestBodyHash: text('pull_request_body_hash').notNull(),
    publicationIntentIdentity: text('publication_intent_identity').notNull().unique(),
    preparedPublicationIdentity: text('prepared_publication_identity').unique(),
    expectedBaseTreeSha: text('expected_base_tree_sha'),
    expectedTreeSha: text('expected_tree_sha'),
    expectedCommitSha: text('expected_commit_sha'),
    state: text('state').notNull(),
    checkpoint: text('checkpoint').notNull(),
    remoteBranchCommitSha: text('remote_branch_commit_sha'),
    githubPullRequestId: bigint('github_pull_request_id', { mode: 'number' }),
    githubPullRequestNumber: integer('github_pull_request_number'),
    githubPullRequestNodeId: text('github_pull_request_node_id'),
    githubPullRequestUrl: text('github_pull_request_url'),
    idempotencyKey: text('idempotency_key').notNull(),
    failureCode: text('failure_code'),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
    preparingStartedAt: timestamp('preparing_started_at', { withTimezone: true }),
    publishingStartedAt: timestamp('publishing_started_at', { withTimezone: true }),
    completedAt: timestamp('completed_at', { withTimezone: true }),
    updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => [
    uniqueIndex('repair_publication_workspace_idempotency_unique').on(table.workspaceId, table.idempotencyKey),
    uniqueIndex('repair_publication_repository_branch_unique').on(table.githubRepositoryId, table.targetBranch),
    index('repair_publication_workspace_created_idx').on(table.workspaceId, table.createdAt),
    check('repair_publication_uuid_check', sql`${table.id} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' and ${table.idempotencyKey} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'`),
    check('repair_publication_version_check', sql`${table.version} = 1`),
    check('repair_publication_hashes_check', sql`${table.humanReviewDecisionIdentity} ~ '^[0-9a-f]{64}$' and ${table.candidateIdentity} ~ '^[0-9a-f]{64}$' and ${table.verificationEvidenceIdentity} ~ '^[0-9a-f]{64}$' and ${table.profileIdentity} ~ '^[0-9a-f]{64}$' and ${table.objectiveContractHash} ~ '^[0-9a-f]{64}$' and ${table.objectiveEvidenceHash} ~ '^[0-9a-f]{64}$' and ${table.repairObjectiveHash} ~ '^[0-9a-f]{64}$' and ${table.pullRequestBodyHash} ~ '^[0-9a-f]{64}$' and ${table.publicationIntentIdentity} ~ '^[0-9a-f]{64}$' and (${table.preparedPublicationIdentity} is null or ${table.preparedPublicationIdentity} ~ '^[0-9a-f]{64}$')`),
    check('repair_publication_git_check', sql`${table.baseCommitSha} ~ '^[0-9a-f]{40}$' and (${table.expectedBaseTreeSha} is null or ${table.expectedBaseTreeSha} ~ '^[0-9a-f]{40}$') and (${table.expectedTreeSha} is null or ${table.expectedTreeSha} ~ '^[0-9a-f]{40}$') and (${table.expectedCommitSha} is null or ${table.expectedCommitSha} ~ '^[0-9a-f]{40}$') and (${table.remoteBranchCommitSha} is null or ${table.remoteBranchCommitSha} ~ '^[0-9a-f]{40}$')`),
    check('repair_publication_state_check', sql`${table.state} in ('queued','preparing','publishing','published','failed','review_required')`),
    check('repair_publication_checkpoint_check', sql`${table.checkpoint} in ('reserved','authority_prepared','objects_verified','branch_create_requested','branch_verified','pr_create_requested','pr_verified','completed')`),
    check('repair_publication_safe_check', sql`${table.targetBranch} ~ '^vigilo/repair/[0-9a-f]{64}$' and char_length(${table.pullRequestTitle}) between 1 and 256 and octet_length(${table.pullRequestBody}) between 1 and 16384 and (${table.failureCode} is null or ${table.failureCode} ~ '^[a-z_]{1,64}$')`),
    check('repair_publication_prepared_facts_check', sql`(${table.preparedPublicationIdentity} is null and ${table.targetBaseBranch} is null and ${table.expectedBaseTreeSha} is null and ${table.expectedTreeSha} is null and ${table.expectedCommitSha} is null) or (${table.preparedPublicationIdentity} is not null and ${table.targetBaseBranch} is not null and char_length(${table.targetBaseBranch}) between 1 and 255 and ${table.expectedBaseTreeSha} is not null and ${table.expectedTreeSha} is not null and ${table.expectedCommitSha} is not null)`),
    check('repair_publication_pr_facts_check', sql`(${table.githubPullRequestId} is null and ${table.githubPullRequestNumber} is null and ${table.githubPullRequestNodeId} is null and ${table.githubPullRequestUrl} is null) or (${table.githubPullRequestId} > 0 and ${table.githubPullRequestNumber} > 0 and char_length(${table.githubPullRequestNodeId}) between 1 and 255 and char_length(${table.githubPullRequestUrl}) between 1 and 2048)`),
    check('repair_publication_state_facts_check', sql`(${table.state} = 'queued' and ${table.checkpoint} = 'reserved' and ${table.preparingStartedAt} is null and ${table.publishingStartedAt} is null and ${table.completedAt} is null and ${table.failureCode} is null) or (${table.state} = 'preparing' and ${table.checkpoint} = 'reserved' and ${table.preparingStartedAt} is not null and ${table.publishingStartedAt} is null and ${table.completedAt} is null and ${table.failureCode} is null) or (${table.state} = 'publishing' and ${table.checkpoint} in ('authority_prepared','objects_verified','branch_create_requested','branch_verified','pr_create_requested','pr_verified') and ${table.preparingStartedAt} is not null and ${table.publishingStartedAt} is not null and ${table.completedAt} is null and ${table.failureCode} is null) or (${table.state} = 'published' and ${table.checkpoint} = 'completed' and ${table.remoteBranchCommitSha} is not null and ${table.githubPullRequestId} is not null and ${table.completedAt} is not null and ${table.failureCode} is null) or (${table.state} = 'failed' and ${table.completedAt} is not null and ${table.failureCode} is not null) or (${table.state} = 'review_required' and ${table.preparedPublicationIdentity} is not null and ${table.completedAt} is not null and ${table.failureCode} is not null)`),
  ],
);

export const repairPublicationAttempt = pgTable(
  'repair_publication_attempt',
  {
    id: text('id').primaryKey(),
    publicationId: text('publication_id').notNull().references(() => repairPublication.id, { onDelete: 'restrict' }),
    queueJobId: text('queue_job_id').notNull(),
    attemptNumber: integer('attempt_number').notNull(),
    ownershipToken: text('ownership_token').notNull(),
    state: text('state').notNull(),
  ...
  (table) => [
    uniqueIndex('repair_publication_attempt_ordinal_unique').on(table.publicationId, table.attemptNumber),
    uniqueIndex('repair_publication_attempt_active_unique').on(table.publicationId).where(sql`${table.state} = 'active'`),
    check('repair_publication_attempt_uuid_check', sql`${table.id} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' and ${table.queueJobId} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' and ${table.ownershipToken} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'`),
    check('repair_publication_attempt_state_check', sql`${table.state} in ('active','succeeded','failed','abandoned') and ${table.attemptNumber} between 1 and 3 and (${table.failureCode} is null or ${table.failureCode} ~ '^[a-z_]{1,64}$')`),
    check('repair_publication_attempt_facts_check', sql`(${table.state} = 'active' and ${table.failureCode} is null and ${table.completedAt} is null) or (${table.state} = 'succeeded' and ${table.failureCode} is null and ${table.completedAt} is not null) or (${table.state} in ('failed','abandoned') and ${table.failureCode} is not null and ${table.completedAt} is not null)`),
  ],
);

export const repairPublicationEvent = pgTable(
  'repair_publication_event',
  {
    id: text('id').primaryKey(),
    publicationId: text('publication_id').notNull().references(() => repairPublication.id, { onDelete: 'restrict' }),
    attemptId: text('attempt_id').references(() => repairPublicationAttempt.id, { onDelete: 'restrict' }),
    workspaceId: text('workspace_id').notNull().references(() => workspace.id, { onDelete: 'restrict' }),
    fromState: text('from_state'),
    toState: text('to_state').notNull(),
    checkpoint: text('checkpoint').notNull(),
  ...
  (table) => [
    index('repair_publication_event_publication_created_idx').on(table.publicationId, table.createdAt),
    check('repair_publication_event_uuid_check', sql`${table.id} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'`),
    check('repair_publication_event_state_check', sql`(${table.fromState} is null or ${table.fromState} in ('queued','preparing','publishing','published','failed','review_required')) and ${table.toState} in ('queued','preparing','publishing','published','failed','review_required')`),
    check('repair_publication_event_checkpoint_check', sql`${table.checkpoint} in ('reserved','authority_prepared','objects_verified','branch_create_requested','branch_verified','pr_create_requested','pr_verified','completed')`),
    check('repair_publication_event_type_check', sql`${table.eventType} in ('reserved','claimed','prepared','objects_verified','branch_requested','branch_verified','pr_requested','pr_verified','completed','failed','review_required','reconciled')`),
    check('repair_publication_event_safe_check', sql`${table.failureCode} is null or ${table.failureCode} ~ '^[a-z_]{1,64}$'`),
    check('repair_publication_event_failure_check', sql`(${table.eventType} in ('failed','review_required') and ${table.failureCode} is not null) or (${table.eventType} not in ('failed','review_required') and ${table.failureCode} is null)`),
  ],
);
...
export const authSchema = {
  ...
  humanReviewDecision,
  repairPublication,
  repairPublicationAttempt,
  repairPublicationEvent,
  session,
  user,
  verification,
  workspace,
