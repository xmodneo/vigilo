...
export const AI_INVESTIGATION_QUEUE = 'ai-investigation-v1';
...
export const REPAIR_PUBLICATION_QUEUE = 'repair-publication-v1';
export const REPAIR_PUBLICATION_JOB_VERSION = 1 as const;
...
export interface AiInvestigationJobPayload {
  version: typeof AI_INVESTIGATION_JOB_VERSION;
  aiInvestigationId: string;
}

export interface AiCandidateGenerationJobPayload {
  version: typeof AI_CANDIDATE_GENERATION_JOB_VERSION;
...
export interface RepairPublicationJobPayload {
  version: typeof REPAIR_PUBLICATION_JOB_VERSION;
...
export interface TransactionalAiInvestigationQueue {
  enqueueAiInvestigation(transaction: VigiloTransaction, payload: AiInvestigationJobPayload): Promise<string>;
}
...
export interface TransactionalRepairPublicationQueue {
  enqueuePublication(transaction: VigiloTransaction, payload: RepairPublicationJobPayload): Promise<string>;
}
...
export function parseAiInvestigationJobPayload(value: unknown): AiInvestigationJobPayload {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('invalid_ai_investigation_job_payload');
  const record = value as Record<string, unknown>;
  if (Object.keys(record).length !== 2 || record.version !== AI_INVESTIGATION_JOB_VERSION || typeof record.aiInvestigationId !== 'string' || !UUID.test(record.aiInvestigationId)) throw new Error('invalid_ai_investigation_job_payload');
  return { version: AI_INVESTIGATION_JOB_VERSION, aiInvestigationId: record.aiInvestigationId };
}

export function parseAiCandidateGenerationJobPayload(value: unknown): AiCandidateGenerationJobPayload {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('invalid_ai_candidate_generation_job_payload');
  const record = value as Record<string, unknown>;
...
export function parseRepairPublicationJobPayload(value: unknown): RepairPublicationJobPayload {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('invalid_repair_publication_job_payload');
  const record = value as Record<string, unknown>;
  if (Object.keys(record).length !== 2 || record.version !== REPAIR_PUBLICATION_JOB_VERSION || typeof record.publicationId !== 'string' || !UUID.test(record.publicationId)) throw new Error('invalid_repair_publication_job_payload');
  return { version: REPAIR_PUBLICATION_JOB_VERSION, publicationId: record.publicationId };
}
...
export class PgBossAiInvestigationQueue implements TransactionalAiInvestigationQueue {
  constructor(private readonly boss: Pick<PgBoss, 'send'>) {}
  async enqueueAiInvestigation(transaction: VigiloTransaction, payload: AiInvestigationJobPayload): Promise<string> {
    const validated = parseAiInvestigationJobPayload(payload);
    const id = await this.boss.send(AI_INVESTIGATION_QUEUE, validated, { ...REPAIR_QUEUE_OPTIONS, db: fromDrizzle(transaction as unknown as DrizzleTransactionLike, sql), id: validated.aiInvestigationId });
    if (id !== validated.aiInvestigationId) throw new Error('ai_investigation_job_not_persisted');
    return id;
  }
}

export class PgBossAiCandidateGenerationQueue implements TransactionalAiCandidateGenerationQueue {
  constructor(private readonly boss: Pick<PgBoss, 'send'>) {}
  async enqueueAiCandidateGeneration(transaction: VigiloTransaction, payload: AiCandidateGenerationJobPayload): Promise<string> {
    const validated = parseAiCandidateGenerationJobPayload(payload);
    const id = await this.boss.send(AI_CANDIDATE_GENERATION_QUEUE, validated, { ...REPAIR_QUEUE_OPTIONS, db: fromDrizzle(transaction as unknown as DrizzleTransactionLike, sql), id: validated.proposalGenerationId });
    if (id !== validated.proposalGenerationId) throw new Error('ai_candidate_generation_job_not_persisted');
    return id;
  }
}
...
export class PgBossRepairPublicationQueue implements TransactionalRepairPublicationQueue {
  constructor(private readonly boss: Pick<PgBoss, 'send'>) {}
  async enqueuePublication(transaction: VigiloTransaction, payload: RepairPublicationJobPayload): Promise<string> {
    const validated = parseRepairPublicationJobPayload(payload);
    const id = await this.boss.send(REPAIR_PUBLICATION_QUEUE, validated, { ...REPAIR_QUEUE_OPTIONS, db: fromDrizzle(transaction as unknown as DrizzleTransactionLike, sql), id: validated.publicationId });
    if (id !== validated.publicationId) throw new Error('repair_publication_job_not_persisted');
    return id;
  }
}
...
export async function configureRepairQueue(boss: Pick<PgBoss, 'createQueue' | 'updateQueue'>): Promise<void> {
  ...
  await boss.createQueue(AI_INVESTIGATION_QUEUE, { policy: 'standard', ...REPAIR_QUEUE_OPTIONS });
  ...
  await boss.createQueue(REPAIR_PUBLICATION_QUEUE, { policy: 'standard', ...REPAIR_QUEUE_OPTIONS });
  await boss.updateQueue(REPAIR_PUBLICATION_QUEUE, REPAIR_QUEUE_OPTIONS);
}
