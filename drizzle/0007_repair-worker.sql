CREATE TABLE "repair_run_attempt" (
	...
	CONSTRAINT "repair_run_attempt_cleanup_check" CHECK (("repair_run_attempt"."cleanup_stop" is null or "repair_run_attempt"."cleanup_stop" in ('confirmed','failed','not_needed')) and ("repair_run_attempt"."cleanup_delete" is null or "repair_run_attempt"."cleanup_delete" in ('confirmed','failed','not_needed')) and ("repair_run_attempt"."cleanup_lookup" is null or "repair_run_attempt"."cleanup_lookup" in ('absent','still_present','unconfirmed','not_run'))),
